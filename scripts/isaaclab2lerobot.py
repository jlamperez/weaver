...
Modified from leisaac/scripts/convert/isaaclab2lerobot.py
...
def process_single_arm_data(dataset: LeRobotDataset, task_description: str, demo_group: h5py.Group, demo_name: str) -> bool:
    try:
...
def process_bi_arm_data(dataset: LeRobotDataset, task_description: str, demo_group: h5py.Group, demo_name: str) -> bool:
    try:
        actions = np.array(demo_group["actions"])
...
def convert_isaaclab_to_lerobot(input_path, repo_id, task_description, push_to_hub):
        ...
        with h5py.File(hdf5_file, "r") as f:
            ...
            for demo_name in tqdm(demo_names, desc="Processing each demo"):
                ...
                if robot_type == "so101_follower":
                    valid = process_single_arm_data(dataset, task_description, demo_group, demo_name)
                elif robot_type == "bi_so101_follower":
                    valid = process_bi_arm_data(dataset, task_description, demo_group, demo_name)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Isaac Lab HDF5 dataset to LeRobot format.")
    parser.add_argument(
        "--input_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        help="The specific task to extract from the HDF5 file."
    )
    parser.add_argument(
        "--push-to-hub",
        action="store_true",
        help="Whether to push the dataset to the Hugging Face Hub after conversion."
    )

    args = parser.parse_args()

    convert_isaaclab_to_lerobot(args.input_path, args.repo_id, args.task_description, args.push_to_hub)
